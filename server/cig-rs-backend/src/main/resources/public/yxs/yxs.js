(function() {
    ...
    const getParams = ()=>{
      ...
      if(navigator.userAgent.toLowerCase().includes("baiduboxapp")) url = decodeURIComponent(url);
    ...
    $('.user-collect-bot-right').click(function(){
      ...
      // 改为后台判断
      var ua = navigator.userAgent.toLowerCase();
      if(ua.includes("toutiaomicroapp") || ua.includes("baiduboxapp")) {
        setModal({
          hintStr:'请在微信搜索小程序【汽车数字化研享社】进行下载'
        })
        return;
      }
      if(urlParams.isAuth != 1){
      ...
      axios.post(`${domin}/report/download`,{
        reportId: urlParams.reportId,
        userId: urlParams.uid,
        platform: ua.includes("micromessenger")?'weapp':'other'
      })
      .then(function (res){
          if(res.status === 200  && res.data.status === 200){
            //下载成功
